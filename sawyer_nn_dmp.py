...
target = np.array([target[0][0], target[0][1], -0.4])
print(target)
...
while error > 0.075:
    ...
    q_in = np.array([[q[0][0], q[0][1], q[0][3], q[0][4]]])
    x = np.divide(forwardModel.predict(q_in), 100)
...
coordinates = [target[0], target[1], 0.1]
